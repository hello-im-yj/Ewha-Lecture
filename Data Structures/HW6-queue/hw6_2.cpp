...
// input: root node
// output: m_LUT, m_bit_size -> 허프만 테이블을 만드는 함수

int arr[MAX_BIT] = { 0 };
void huffman_traversal(TreeNode* node,int level)
{	
	if (node->l == NULL && node->r == NULL) { //leafnode일 경우
		m_bit_size[node->data - 'a'] = level;
		for (int i = 0; i < level; i++) {
			printf("%c", node->data);
			m_LUT[node->data - 'a'][i] = arr[i];
		}
	}
		...
		if (node->l != NULL) {
			arr[level] = 0;
			huffman_traversal(node->l,level+1);
		}

		if (node->r != NULL) {
			arr[level] = 1;
			huffman_traversal(node->r,level+1);
		}
	}
...
int huffman_decoding(bits_stream* bits_str, TreeNode* node, char* decoded_str)
{
	...
	for (int i = 0; i < bits_str->length; i++) {
		...
		if (here->l == NULL && here->r == NULL) {
			decoded_str[index_char++] = here->data;
			here = node;
		}
	}
...
void main()
{
	...
	// m_bit_size = {1, 3, ...}
	m_LUT = mem_2D_int(m_char_size, MAX_BIT); //10*6 2차원 배열
	m_bit_size = (int*)malloc(sizeof(int) * m_char_size); //1차원 int 배열 6칸
	...
	//printf(NULL== element_root.ptree->l->l ? "TRUE": "FALSE");

	// Generate the huffman codeword from the huffman binary tree -> m_LUT와 m_bit_size 완성
